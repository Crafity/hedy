...
window.auth = {
  ...
  logout() {
    $.ajax({ type: 'POST', url: '/auth/logout' }).done(() => {
      auth.redirect('login');
    });
  },
  destroy() {
    window.modal.confirm(auth.texts.are_you_sure, () => {
      $.ajax({ type: 'POST', url: '/auth/destroy' }).done(() => {
        auth.redirect('');
      });
    });
  },
  ...
  submit(op) {
    ...
    if (op === 'signup') {
      ...
      var payload = {};
      ['username', 'email', 'password', 'birth_year', 'country', 'gender', 'subscribe'].map(k => {
        if (!values[k]) {
          return;
        }
        if (k === 'birth_year') {
      ...
      $.ajax({
        ...
        .done(() => {
          ...
          let savedProgram = localStorage.getItem('hedy-first-save');
          let joinClass = localStorage.getItem('hedy-join');
          if (!savedProgram) {
            if (!joinClass) {
          ...
          window.saveit(savedProgram[0], savedProgram[1], savedProgram[2], savedProgram[3], () => {
      ...
      $.ajax({
        ...
        .fail(response => {
          if (response.status === 403) {
            auth.error(
              `${auth.texts.invalid_username_password} ${auth.texts.no_account} &nbsp;<button class="green-btn" onclick="auth.redirect ('signup')">${auth.texts.create_account}</button>`
            );
            $('#create-account').hide();
    ...
    if (op === 'profile') {
      ...
      var payload = {};
      ['email', 'birth_year', 'country', 'gender'].map(k => {
        if (!values[k]) {
          return;
        }
        if (k === 'birth_year') {
  ...
  markAsTeacher(username, is_teacher) {
    $.ajax({
      type: 'POST',
      url: '/admin/markAsTeacher',
      data: JSON.stringify({ username, is_teacher }),
      contentType: 'application/json; charset=utf-8',
    })
      .done(() => {
        window.modal.alert(
          ['User', username, 'successfully', is_teacher ? 'marked' : 'unmarked', 'as teacher'].join(' '),
          4000
        );
        location.reload();
      })
      .fail(error => {
        console.log(error);
        window.modal.alert(
          ['Error when', is_teacher ? 'marking' : 'unmarking', 'user', username, 'as teacher'].join(' ')
        );
      });
  },

  changeUserEmail(username, email) {
    window.modal.prompt('Please enter the corrected email', email, correctedEmail => {
      ...
      $.ajax({
        ...
        data: JSON.stringify({ username, email: correctedEmail }),
        contentType: 'application/json; charset=utf-8',
      })
        .done(() => {
...
$.ajax({ type: 'GET', url: '/profile' })
  .done(response => {
    ...
    if ($('#profile').html()) {
      ...
      (response.experience_languages || []).map(lang => {
        $(`input[name=languages][value="${lang}"]`).prop('checked', true);
      });
      $('#student_classes ul').html(
        (response.student_classes || []).map(Class => `<li>${auth.entityify(Class.name)}</li>`).join('')
      );
    }
  })
  .fail(response => {
...
$('#email, #mail_repeat').on('cut copy paste', e => {
  e.preventDefault();
  return false;
});
